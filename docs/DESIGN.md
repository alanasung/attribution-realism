# Design Document: When Data Attribution Breaks Outside Toy Settings

## 1. Purpose

This document is the method contract for **When Data Attribution Breaks Outside Toy Settings**. It specifies what is
...
One-liner: Measure where influence-style attribution degrades as data and model settings become more realistic.

Hypothesis: Measure where influence-style attribution degrades as data and model settings become more realistic.
...
|---|---|---|
| Config schema | `attrreal.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `attrreal.configs.loader` | Hydra compose API |
| Device | `attrreal.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `attrreal.models.registry` | Pinned revisions + hardware fit |
| Hooks | `attrreal.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `attrreal.models.generation` | Chat template path recording |
| Data | `attrreal.data` | Synthetic items, splits, manifests |
| Evaluation | `attrreal.evaluation` | Metrics, failure harness, protocols |
| Ablation | `attrreal.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `attrreal.reporting` | Aggregate → MD/TeX/figures |
| Cache | `attrreal.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/attribution-realism
...
`src/attrreal/attrreal/` are preserved across regeneration.
